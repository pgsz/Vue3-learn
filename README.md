...
## 组件库
  ...
  - 布局组件
    - 基础组件：项目中用到的组件规范，包含布局、色彩、字体、图标等。
    - 表单组件：负责用户的输入数据管理，包含常见的输入框、滑块、评分等；总结说：需要用户输入的地方就是表单组件的应用场景，其中对用户输入的校验是重要的功能点。
    - 数据组件：负责显示后台的数据，重要的就是表格和树形组件。
    - 通知组件：负责通知用户操作的状态，包含警告和弹窗；如何用函数动态渲染组件是重要的功能点。
    - 导航组件：
    - 其他组件
